...
function test() {
    const keys = ['secret', 's', '1', 2, 3.231, 'pifjiodsjifOES', '*ASUDO09AU()D*AUD09\\::DSK;DSK;KALDJLMMSMKLAMDEWU903U9031MALSDMSAMDIOJDW'] // random strings, ints, floats, etc.
    const delimiters = ['.', ',', '-', ' ', ';', '=', ':', '+', '*', '&', '^', '%', '$', '#', '@', '!', '~', '`', '|', '\\', '/', '?', '>', '<', ')', '(', ']', '[', '}', '{', ']', '[']
        ...
        delimiters.forEach((d) => {
            datas.forEach((data) => {
                xs.forEach((x) => {
                    ...
                    if (data != decoded) {
                        console.log(`\n\nError: encoded does not equal decoded.\nKey: ${k}\nDelimiter: ${d}\nX: ${x}\nData: ${data}\nEncoded: ${encoded}\nDecoded: ${decoded}\n`)
                        return
                    }
                    passed ++
                })
